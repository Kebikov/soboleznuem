...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // //получение карточек
    // const shopBlockItems = document.querySelectorAll('.shop-block__item');
    // let cartArr = [];
    // shopBlockItems.forEach(item => {
    //     let objectCart = {};
    //     objectCart.title = item.querySelector('.card__title').innerHTML;
    //     objectCart.img1 = item.querySelector('source').getAttribute('srcset');
    //     objectCart.img2 = item.querySelector('img').getAttribute('src');
    //     objectCart.prais = item.querySelector('.card__prais').innerHTML;
    //     cartArr.push(objectCart);
    // });
    // const jsonCardArr = JSON.stringify(cartArr);
    // //console.log('',jsonCardArr);
